...
class EvaluationRecord:
    ...
    def compute(self) -> Dict[str, Any]:
        ...
        self.extra_alerts.clear()
        record = deepcopy(self.payload)

        resolved_weights = self._resolve_weights(record.get("pesos"))
        self.weights = resolved_weights
        record["pesos"] = deepcopy(resolved_weights)
        ...
        alerts.update(self.extra_alerts)
    ...
    def _resolve_weights(self, provided: Any) -> Dict[str, float]:
        """Return a validated weight mapping based on user input."""

        weights = deepcopy(CATEGORY_WEIGHTS)

        if isinstance(provided, dict):
            for category, default_weight in CATEGORY_WEIGHTS.items():
                candidate = provided.get(category, default_weight)
                if isinstance(candidate, (int, float)):
                    weights[category] = float(candidate)
                else:
                    try:
                        weights[category] = float(candidate)
        ...
        if total <= 0:
            self.extra_alerts.append("inconsistencia_kpi")
            return deepcopy(CATEGORY_WEIGHTS)
...
def dump_to_file(evaluations: List[Dict[str, Any]], output_path: str) -> None:
    """Write the evaluations list to ``output_path`` as JSON."""
